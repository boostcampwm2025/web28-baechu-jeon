...
1단계(주요 파일 추천) 결과, 프로젝트 구조, 그리고 기존 파일 내용을 바탕으로 *폴더*와 *주요 파일*이 프로젝트 내에서 어떤 역할을 하는지 설명해야 합니다.
분석은 *파일은 1단계 결과 안에 있는 파일 경로와 파일들의 상위(부모, 조부모, … 최상위까지)폴더만 하세요. 1단계 결과에 없는 파일은 포함시키지 마세요*
# 분석 대상
...
2. *파일:* 1단계 결과에 있는 파일들만 모두 \`path\`에 넣으세요. 1단계 결과에 없는 파일은 포함시키지 마세요.
*중요 제한:*
- 파일 path는 1단계 결과 안에 있는 파일들을 모두 넣습니다. *1단계 결과에 포함되지 않은 파일은 절대 포함시키지 마세요.*
- *트리 범위:* path에는 꼭 *1단계에서 분석한 주요 파일*이 들어 있는 폴더·파일만 포함하세요. *제외할 것:* 설정·CI·배포·스크립트·인프라 전용 폴더(예: \.github\, \scripts\, Docker/nginx 설정용 폴더)와 루트 설정 파일(예: \docker-compose.yml\, \compose.local.yml\). 프로젝트 구조가 모노레포인지 단일 앱인지에 따라 "앱 코드 영역"이 다르므로, *비즈니스/앱 소스가 있는 경로만* 포함하면 됩니다.
- *트리 연결 필수:*  분석결과를 이용하여 트리구조를 그릴려고 합니다 따라서 최상위와 1단계 파일 사이 폴더:** 각 \project_main_files\ 경로에서 *위에서 정한 최상위 폴더부터 그 파일까지* 사이에 있는 모든 폴더를 포함하세요.예: \`apps/frontend/src/entities/node/model.js\을 넣으면 \apps/frontend/src/entities/node\, \apps/frontend/src/entities\, \apps/frontend/src\, \apps/frontend\`도 리스트에 있어야 합니다. *중간 경로를 생략하면 해당 하위 노드들이 트리에서 붕 떠 보이므로, 상위 경로를 누락하지 마세요.*
# path 필드
...
- *user_stories의 related_paths에는 responsibility_hypotheses의 path 중 파일 경로만* 넣으세요. (폴더 경로는 제외) path와 완전히 동일한 문자열만** 사용하세요.
{
