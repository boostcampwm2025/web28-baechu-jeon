...
2. **파일:** 1단계 \`project_main_files\`에 나열된 **파일 경로만** + 그 상위 폴더(트리에서 상위 노드 클릭 시에도 상세 정보가 있도록). 이 파일들은 "1단계에서 요청한 주요 파일 내용"으로 소스가 제공되므로, 이 경로들을 \`path\`에 넣고 각각 hypothesis를 작성하세요. 목록에 없는 파일은 추가하지 마세요.
