...
- **실제 동작·로직·규칙이 구현된 파일**(API·핵심 기능·비즈니스 흐름이 드러나는 파일)도 함께 포함하세요. "무엇으로 구성되어 있는지"와 "무엇을 하는지"가 모두 드러나도록 선택하세요.
- 프로젝트 이해에 도움이 되는 핵심 기능과 비지니스 로직이 있는 파일들을 최대한 골라 주세요.
- 개수는 너무 과하지않게 적당하게 골라 주세요.
